...
function SidebarGroupLabel({ className, render, ...props }: useRender.ComponentProps<"div">) {
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex h-8 shrink-0 items-center rounded-md bg-linear-to-r from-transparent to-transparent px-2 text-xs font-medium text-muted-foreground ring-ring outline-hidden transition-[margin,opacity] duration-300 ease-linear",
