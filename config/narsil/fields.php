<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Available Fields
    |--------------------------------------------------------------------------
    |
    | Field contracts available as Fields.
    |
    */

    \Narsil\Contracts\Fields\ArrayField::class,
    \Narsil\Contracts\Fields\BuilderField::class,
    \Narsil\Contracts\Fields\CheckboxField::class,
    \Narsil\Contracts\Fields\DateField::class,
    \Narsil\Contracts\Fields\DatetimeField::class,
    \Narsil\Contracts\Fields\EmailField::class,
    \Narsil\Contracts\Fields\EntityField::class,
    \Narsil\Contracts\Fields\FileField::class,
    \Narsil\Contracts\Fields\FormField::class,
    \Narsil\Contracts\Fields\LinkField::class,
    \Narsil\Contracts\Fields\NumberField::class,
    \Narsil\Contracts\Fields\PasswordField::class,
    \Narsil\Contracts\Fields\RangeField::class,
    \Narsil\Contracts\Fields\RelationsField::class,
    \Narsil\Contracts\Fields\RichTextField::class,
    \Narsil\Contracts\Fields\SelectField::class,
    \Narsil\Contracts\Fields\SwitchField::class,
    \Narsil\Contracts\Fields\TableField::class,
    \Narsil\Contracts\Fields\TextField::class,
    \Narsil\Contracts\Fields\TextareaField::class,
    \Narsil\Contracts\Fields\TimeField::class,
    \Narsil\Contracts\Fields\TreeField::class,
];
